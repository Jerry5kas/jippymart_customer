...
<div class="relative shadow-md hover:shadow-xl transition-all duration-300 rounded-xl bg-white overflow-hidden group w-full h-full">
    ...
    <div class="relative">
        ...
        <div x-data="martCartItem('{{ addslashes($title) }}', {{ $disPrice }}, {{ $price }}, '{{ addslashes($src) }}', '{{ addslashes($subcategoryTitle) }}', '{{ addslashes(str_replace(["\r\n", "\r", "\n"], ' ', $description)) }}', '{{ $grams }}', {{ $rating }}, {{ $reviews }})" 
             x-cloak
