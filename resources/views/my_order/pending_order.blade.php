                ...
                <ul class="nav nav-tabsa custom-tabsa border-0 bg-white rounded overflow-hidden shadow-sm p-2 c-t-order"
                    ...
                    <li class="nav-item border-top" role="presentation">
                        <a class="nav-link border-0 text-dark py-3 active"
                            href="{{ url('my_order') }}?activeTab=progress"> <i
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                ...
                                <div class="p-3 border-bottom">
                                    ...
                                    <img onerror="this.onerror=null;this.src=\'' + place_image + '\'" alt="#"
                                        src="img/logo_web.png" class="img-fluid sc-siddhi-logo mr-2"><span
        ...
        completedorsersref.get().then(async function(completedorderSnapshots) {
            ...
            if (orderDetails.author.id != user_uuid) {
                ...
                for (let i = 0; i < orderDetails.products.length; i++) {
                    ...
                    var productPriceTotal = parseFloat(orderDetails.products[i]['price']) * parseInt(
                ...
                if (orderDetails.hasOwnProperty('specialDiscount') && orderDetails.specialDiscount) {
                    if (orderDetails.specialDiscount != null) {
                        if (orderDetails.specialDiscount.specialType != "" && orderDetails
                            .specialDiscount.specialType != null) {
                        ...
                        if (!isNaN(tax) && tax != 0) {
                            ...
                            if (currencyAtRight) {
                                $("#order-tax").append(
                                    ...
                                    parseFloat(tax).toFixed(decimal_degits) + '' + currentCurrency +
                                    "</h6></div>");
                            } else {
                                $("#order-tax").append(
                                    '<div class="d-flex align-items-center mb-2"><h6 class="font-weight-bold  mb-1">' +
                ...
                if (currencyAtRight) {
                    ...
                    order_subtotal_main = order_subtotal_main.toFixed(decimal_degits) + "" +
                        currentCurrency;
                    order_shipping_val = parseFloat(order_shipping).toFixed(decimal_degits) + "" +
                        currentCurrency;
                    order_discount_val = parseFloat(order_discount).toFixed(decimal_degits) + "" +
                        currentCurrency;
                    order_tip_amount_val = parseFloat(order_tip_amount).toFixed(decimal_degits) + "" +
                        currentCurrency;
                    order_special_discount = parseFloat(special_discount).toFixed(decimal_degits) + "" +
                        currentCurrency;
                } else {
                    ...
                    order_tip_amount_val = currentCurrency + "" + parseFloat(order_tip_amount).toFixed(
                        decimal_degits);
                    order_special_discount = currentCurrency + "" + parseFloat(special_discount)
                        .toFixed(decimal_degits);
                }
                ...
                if (orderDetails.hasOwnProperty('estimatedTimeToPrepare') && orderDetails
                    .estimatedTimeToPrepare != null) {
                    if (orderDetails.hasOwnProperty('estimatedTimeToPrepare') && orderDetails
                        .estimatedTimeToPrepare != null) {
                        prepareTime = orderDetails.estimatedTimeToPrepare;
                        var [h, m] = prepareTime.split(":");
                        time = (h == "00") ? m + " minutes" : h + " hours" + m + " minutes";
                        $('.estimated_pre_time').append(
                            '<label class="col-12 control-label pl-0"><strong>{{ trans('lang.estimated_pre_time') }}:</strong><span id=""> ' +
                            time + '</span></label>')
                        $(".estimated_pre_time").show();
                    }
                }
