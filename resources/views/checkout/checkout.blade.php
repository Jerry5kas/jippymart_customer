...
<script type="text/javascript">
        ...
        $(document).on("click", '#apply-coupon-code', function(event) {
            ...
            couponCodeRef.get().then(async function(couponSnapshots) {
                if (couponSnapshots.docs.length) {
                    ...
                    var item_value = coupondata.item_value || 0; // <-- fetch item_value from Firestore
                    var to_pay = parseFloat($("#total_pay").val()); // get current cart total

                    if (to_pay >= item_value) {
                        // Coupon is valid, apply as before
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                if (data.status) {
                                    ...
                                    $("#coupon_code").val('');
                                    loadcurrencynew();
                                    $("#coupon-message").html('<div class="alert alert-success mt-2">' + (data.message || 'Coupon is valid and applied!') + '</div>');
                                    var today = new Date().toISOString().slice(0, 16);
                                    if (document.getElementsByName("scheduleTime").length > 0) {
                                        document.getElementsByName("scheduleTime")[0].min = today;
                                    }
                                    ...
                                    $("#coupon-message").html('<div class="alert alert-danger mt-2">' + (data.message || 'Coupon is not applied.') + '</div>');
                                }
                            }
                        });
                    } else {
                        // Coupon is not valid, show message
                        $("#coupon-message").html('<div class="alert alert-danger mt-2">Order total is too low for this coupon. Minimum required: ' + item_value + '</div>');
                    }
                } else {
                    Swal.fire({
    ...
    async function getUserDetails() {
        codSettings.get().then(async function(codSettingsSnapshots) {
            var codSetting = codSettingsSnapshots.data();
            if (codSetting.isEnabled) {
                $("#cod_box").show();
            }
        });
    ...
    async function finalCheckout() {
        ...
        // Check COD order limit
        if (payment_method == "cod") {
            payableAmount = $('#total_pay').val();
            if (parseFloat(payableAmount) > 599) {
                Swal.fire({
                    text: "Cash on Delivery orders cannot exceed ₹600. Please choose another payment method.",
                    icon: "error"
                });
                return false;
            }
    ...
    $.ajax({
        ...
        data: {
            ...
            currencyData: currencyData
                            ...
                            // Add a small delay to ensure order is created in Firebase
                            setTimeout(function() {
                            ...
                            $.ajax({
                                ...
                                success: async function(data) {
                                    if (payment_method == "wallet") {
                                        ...
                                        $.ajax({
                                            type: 'POST',
                                            url: "<?php echo route('store-order-session'); ?>",
                                            data: {
                                                _token: '<?php echo csrf_token(); ?>',
                                                order_id: id_order,
                                                payment_method: payment_method
